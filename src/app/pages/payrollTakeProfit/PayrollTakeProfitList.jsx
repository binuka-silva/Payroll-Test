...
import payrollTakeProfitService from "../../api/payrollTakeProfitServices/payrollTakeProfitService";
...
const PayrollTakeProfitList = ({
          ...
          },
          {
            ...
            validate: (rowData) =>
              rowData.sequence === undefined
                ...
                : payrollTakeProfitList.find((c) =>c.sequence === parseInt(rowData.sequence) &&c.id !== rowData.id)
                ? { isValid: false, helperText: "Sequence was already used" }
                : true,
          },
        ]);
    }, [payrollPayItemData, payItem,payrollTakeProfitList]);
    ...
    const addRow = (newRow) =>
        new Promise((resolve, reject) => {
            ...
            payrollTakeProfitService().create(payrollTakeProfit)
                .then(async (response) => {
    ...
    const updateRow = (editedRow, prevRow) =>
        new Promise((resolve, reject) => {
            ...
            payrollTakeProfitService().update(payrollTakeProfit, editedRow.id)
                .then(async (response) => {
                    NotificationManager.success(
    ...
    const deleteRow = (deletedRow) =>
        new Promise(async (resolve, reject) => {
            ...
            await payrollTakeProfitService()
                .deletePayrollTakeProfit(payrollTakeProfit)
                .then((response) => {
                    //Reload list
                    fetchPayrollTakeProfitFunc();
                    resolve();
    ...
    const isExists = (row) => {
        ...
        const existPayItems = payrollTakeProfitList.find((detail) => detail.payItem === x);
        const existSequnces = payrollTakeProfitList.find((detail) => detail.sequence === parseInt(row.sequence));

import React, {forwardRef, useEffect, useState} from "react";

import MaterialTable from "@material-table/core";

import AddBox from "@material-ui/icons/AddBox";
import Check from "@material-ui/icons/Check";
import Clear from "@material-ui/icons/Clear";
import DeleteOutline from "@material-ui/icons/DeleteOutline";
import ChevronRight from "@material-ui/icons/ChevronRight";
import Edit from "@material-ui/icons/Edit";
import SaveAlt from "@material-ui/icons/SaveAlt";
import FilterList from "@material-ui/icons/FilterList";
import FirstPage from "@material-ui/icons/FirstPage";
import LastPage from "@material-ui/icons/LastPage";
import ChevronLeft from "@material-ui/icons/ChevronLeft";
import ArrowDownward from "@material-ui/icons/ArrowDownward";
import Remove from "@material-ui/icons/Remove";
import ViewColumn from "@material-ui/icons/ViewColumn";
import {Search} from "@material-ui/icons";

import {NotificationManager} from "react-notifications";
import loanTypeService from "../../api/LoanTypeServices/loanTypeService";
import localStorageService from "../../services/localStorageService";
import {NOTIFICATION_ERROR} from "../../common/notifications";
import {RESP_STATUS_CODES} from "../../common/response";
import handlePageSize from "../../common/tablePageSize";
import AutoCompleteDropDown from "../../components/AutoCompleteDropDown";

const LoanTypeList = ({
                          fetchLoanTypeDataFunc,
                          loanTypeList,
                          isLoading,
                          //fetchCalculationLogicData,
                          //calculationLogicData,
                      }) => {
    const [calculationLogic, setCalculationLogic] = useState("");
    const [calculationLogicData, setCalculationLogicData] = useState([]);

    //Table Columns
    const [columns, setColumns] = useState([]);

    //Component did mount only
    useEffect(() => {
        //Fetch table data
        fetchCalculationLogicData();
    }, []);

    useEffect(() => {
        // Clear all notifications
        NotificationManager.listNotify.forEach((n) =>
            NotificationManager.remove({id: n.id})
        );
    }, []);

    //Load data
    function fetchCalculationLogicData() {
        loanTypeService().getAllLoanCalculationLogicTypes().then((response) => {
            let loanCalculationDataArray = [];
            response.data.forEach((item) => {
                loanCalculationDataArray.push({value: item.id, label: item.name});
            });
            setCalculationLogicData(loanCalculationDataArray);
        });
    }

    const tableIcons = {
        Add: forwardRef((props, ref) => <AddBox {...props} ref={ref}/>),
        Check: forwardRef((props, ref) => <Check {...props} ref={ref}/>),
        Clear: forwardRef((props, ref) => <Clear {...props} ref={ref}/>),
        Delete: forwardRef((props, ref) => <DeleteOutline {...props} ref={ref}/>),
        DetailPanel: forwardRef((props, ref) => (
            <ChevronRight {...props} ref={ref}/>
        )),
        Edit: forwardRef((props, ref) => <Edit {...props} ref={ref}/>),
        Export: forwardRef((props, ref) => <SaveAlt {...props} ref={ref}/>),
        Filter: forwardRef((props, ref) => <FilterList {...props} ref={ref}/>),
        FirstPage: forwardRef((props, ref) => <FirstPage {...props} ref={ref}/>),
        LastPage: forwardRef((props, ref) => <LastPage {...props} ref={ref}/>),
        NextPage: forwardRef((props, ref) => <ChevronRight {...props} ref={ref}/>),
        PreviousPage: forwardRef((props, ref) => (
            <ChevronLeft {...props} ref={ref}/>
        )),
        ResetSearch: forwardRef((props, ref) => <Clear {...props} ref={ref}/>),
        Search: forwardRef((props, ref) => <Search {...props} ref={ref}/>),
        SortArrow: forwardRef((props, ref) => (
            <ArrowDownward {...props} ref={ref}/>
        )),
        ThirdStateCheck: forwardRef((props, ref) => (
            <Remove {...props} ref={ref}/>
        )),
        ViewColumn: forwardRef((props, ref) => <ViewColumn {...props} ref={ref}/>),
    };

    //Table Columns
    useEffect(() => {
        setColumns([
            {
                title: "Id",
                field: "id",
                hidden: true,
            },
            {
                title: "Created Date Time",
                field: "createdDateTime",
                hidden: true,
            },
            {
                title: "Created By",
                field: "createdBy",
                hidden: true,
            },
            {
                title: "Code",
                field: "code",
                validate: (rowData) =>
                    rowData.code === undefined
                        ? {isValid: false, helperText: "Loan Type code is required"}
                        : rowData.code === ""
                            ? {isValid: false, helperText: "Loan Type code is required"}
                            : true,
                editable: "onAdd",
            },
            {
                title: "Name",
                field: "name",
                validate: (rowData) =>
                    rowData.name === undefined
                        ? {isValid: false, helperText: "Loan Type Name is required"}
                        : rowData.name === ""
                            ? {isValid: false, helperText: "Loan Type Name is required"}
                            : true,
            },
            {
                title: "Calculation Logic",
                field: "calculationLogic",
                editComponent: (props) => {
                    let x = calculationLogicData.find((b) => b.label === props.value);
                    x && setCalculationLogic(x.value);
                    return (
                        <AutoCompleteDropDown
                            dropDownData={calculationLogicData}
                            onChange={(e, selected) => {
                                setCalculationLogic(selected?.value);
                                props.onChange(selected);
                            }}
                            label="Select"
                            defaultValue={x ?? props.value}
                        />
                    );
                },
            },
            {
                title: "Maximum Amount",
                field: "maxAmount",
                type: "numeric",
                validate: (rowData) =>
                    rowData.maxAmount === undefined
                        ? {isValid: false, helperText: "Maximum Amount is required."}
                        : rowData.maxAmount === ""
                            ? {isValid: false, helperText: "Maximum Amount is required."}
                            : true,
            },
            {
                title: "Maximum Instalments",
                field: "maxInstalments",
                type: "numeric",
                validate: (rowData) =>
                    rowData.maxInstalments === undefined
                        ? {isValid: false, helperText: "Maximum Instalments is required."}
                        : rowData.maxInstalments === ""
                            ? {isValid: false, helperText: "Maximum Instalments is required."}
                            : true,
            },
            {
                title: "Interest Rate",
                field: "interestRate",
                type: "numeric",
                validate: (rowData) =>
                    rowData.interestRate === undefined
                        ? {isValid: false, helperText: "Interest Rate is required."}
                        : rowData.interestRate === ""
                            ? {isValid: false, helperText: "Interest Rate is required."}
                            : true,
            },
            {
                title: "Active Loan",
                field: "active",
                type: "boolean",
                color: "primary",
            },
            {
                title: "Allow Multiple Applications",
                field: "allowMultiple",
                type: "boolean",
                color: "primary",
            },
        ]);
    }, [calculationLogicData]);

    //Add Row
    const addRow = async (newRow) =>
        new Promise(async (resolve, reject) => {
            //Obj Create
            let loanType = {
                code: newRow.code,
                name: newRow.name,
                calculationLogic: calculationLogic,
                maxAmount: newRow.maxAmount,
                maxInstalments: newRow.maxInstalments,
                interestRate: newRow.interestRate,
                active: newRow.active ?? false,
                allowMultiple: newRow.allowMultiple ?? false,
                IsDelete: false,
                createdDateTime: new Date(),
                createdBy: "3fa85f64-5717-4562-b3fc-2c963f66afa6",
            };
            await loanTypeService()
                .create(loanType)
                .then(async (response) => {
                    NotificationManager.success(
                        "A record was successfully created",
                        "Success"
                    );
                    //Reload list
                    fetchLoanTypeDataFunc();
                    resolve();
                })
                .catch((error) => {
                    console.log(error);
                    reject();
                    if (
                        error.status === RESP_STATUS_CODES.FORBIDDEN ||
                        error.status === RESP_STATUS_CODES.UNAUTHORIZED
                    ) {
                        NotificationManager.error(
                            NOTIFICATION_ERROR.AUTH_FAILED,
                            error.statusText
                        );
                    } else {
                        NotificationManager.error(
                            "An existing record already found",
                            "Error"
                        );
                    }
                });
        });

    //Update Row
    const updateRow = (editedRow) =>
        new Promise(async (resolve, reject) => {
            //Obj Create
            let loanType = {
                id: editedRow.id,
                code: editedRow.code,
                name: editedRow.name,
                calculationLogic: calculationLogic,
                maxAmount: editedRow.maxAmount,
                maxInstalments: editedRow.maxInstalments,
                interestRate: editedRow.interestRate,
                active: editedRow.active ?? false,
                allowMultiple: editedRow.allowMultiple ?? false,
                IsDelete: false,
                createdDateTime: editedRow.createdDateTime,
                createdBy: editedRow.createdBy,
                modifiedDateTime: new Date(),
                modifiedBy: "5fa85f64-5717-4562-b3fc-2c963f66a456",
            };
            await loanTypeService()
                .update(loanType)
                .then(async (response) => {
                    NotificationManager.success(
                        "A record was successfully updated",
                        "Success"
                    );
                    //Reload list
                    fetchLoanTypeDataFunc();
                    resolve();
                })
                .catch((error) => {
                    console.log(error);
                    reject();
                    if (
                        error.status === RESP_STATUS_CODES.FORBIDDEN ||
                        error.status === RESP_STATUS_CODES.UNAUTHORIZED
                    ) {
                        NotificationManager.error(
                            NOTIFICATION_ERROR.AUTH_FAILED,
                            error.statusText
                        );
                    } else {
                        NotificationManager.error(
                            "An existing record already found",
                            "Error"
                        );
                    }
                });
        });

    //Delete Row
    const deleteRow = (deletedRow) =>
        new Promise(async (resolve, reject) => {
            //Obj Create
            let loanType = {
                id: deletedRow.id,
            };
            await loanTypeService()
                .deleteLoanType(loanType)
                .then(async (response) => {
                    //Reload list
                    fetchLoanTypeDataFunc();
                    resolve();
                    NotificationManager.success(
                        "A record was successfully deleted.",
                        "Success"
                    );
                })
                .catch((error) => {
                    console.log(error);
                    reject();
                    if (
                        error.status === RESP_STATUS_CODES.FORBIDDEN ||
                        error.status === RESP_STATUS_CODES.UNAUTHORIZED
                    ) {
                        NotificationManager.error(
                            NOTIFICATION_ERROR.AUTH_FAILED,
                            error.statusText
                        );
                    } else {
                        NotificationManager.error(
                            "An existing record already found",
                            "Error"
                        );
                    }
                });
        });

    const fetchTableData = async (query) => {

        let loanCalculationDataArray = [];
        await loanTypeService().getAllLoanCalculationLogicTypes().then((response) => {
            response.data.forEach((item) => {
                loanCalculationDataArray.push({value: item.id, label: item.name});
            });
        });

        const tableData = loanTypeService()
            .getAllByPagination(
                query.page + 1,
                query.pageSize,
                query.orderBy?.field,
                query.orderDirection != "desc",
                query.search,
                query.filters
            )
            .then(({data}) => {
                var mappedData = data.loanType.map((item) => ({
                    id: item.id,
                    code: item.code,
                    name: item.name,
                    calculationLogic: loanCalculationDataArray?.find((v) => v.value === parseInt(item.calculationLogic))?.label,
                    maxAmount: item.maxAmount,
                    maxInstalments: item.maxInstalments,
                    interestRate: item.interestRate,
                    active: item.active,
                    allowMultiple: item.allowMultiple,
                }));
                return {
                    data: mappedData,
                    page: query.page,
                    totalCount: data.dbSize,
                };
            });
        return tableData;
    };

    return (
        <>
            <MaterialTable
                icons={tableIcons}
                title=""
                columns={columns}
                //data={loanTypeList}
                data={(query) => fetchTableData(query)}
                editable={{
                    onRowAdd: (newRow) => addRow(newRow),
                    onRowUpdate: (editedRow) => updateRow(editedRow),
                    onRowDelete: (deletedRow) => deleteRow(deletedRow),
                }}
                options={{
                    addRowPosition: "first",
                    actionsColumnIndex: -1,
                    filtering: true,
                    pageSizeOptions: [5, 10, 20, 50, 100],
                    pageSize:
                        JSON.parse(
                            localStorageService.getItem("auth_user")?.tablePageCount ?? null
                        )?.[window.location.pathname] ?? 5,
                    emptyRowsWhenPaging: false,
                }}
                onRowsPerPageChange={(pageSize) =>
                    handlePageSize(pageSize, window.location.pathname)
                }
                isLoading={isLoading}
            />
        </>
    );
};

export default LoanTypeList;
